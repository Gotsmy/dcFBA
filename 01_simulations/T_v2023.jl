...
function fed_batch_dFBA(N0,V0,nFE,S,t_max,t_min,V_max,c_G,glu,atp,so4,xxx,pro,vlb,vub,d)
    A_idx, B_idx, C_idx, D_idx = fake_get_idx_lists(vlb,vub)
    nA = size(A_idx,1)
    nB = size(B_idx,1)
    nC = size(C_idx,1)
    nD = size(D_idx,1)
    ...
    @variables(m, begin
            ...
            tFE[1:nFE]  # length of finite elements [-]
    ...
    for i in 1:nFE
        set_start_value(tFE[i], t_max/nFE)
    end
    ...
    tFE_min = 1.3 # h
    for i in 1:nFE
        set_lower_bound(tFE[i],tFE_min)
        set_upper_bound(tFE[i],tFE_max)
    ...
    @JLD2.save dirname*"/variables.jld2" N_ Ndot_ V_ Vdot_ q_ lambda_Sv_ tFE_ F_ S0_ t_end_ alpha_ub_A_ slack_ub_A_ alpha_lb_A_ slack_lb_A_ alpha_lb_B_ slack_lb_B_ alpha_ub_C_ slack_ub_C_
